...
public class InnerHandler
{
    ...
    /// </summary>
    /// <param name="request">The socket message request containing the UpdateUserConnectionIdCommand.</param>
    /// <returns>The APIGatewayProxyResponse.</returns>
    public async Task<APIGatewayProxyResponse> Handle(SocketMessage<UpdateUserConnectionIdCommand> request)
    {
        // Check if the request or its message is null
        if (request?.Message is null)
            throw new BadRequestException("Unable to parse request.", typeof(UpdateUserConnectionIdCommand));
